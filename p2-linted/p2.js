$(function() {
  ...
  const add4 = '#add4';
  const add5 = '#add5';
  const extraHTML = '<button id="start-search">SEARCH ADDRESS</button><br /><div id="address"></div>';
    ...
    if ($(searchField).val() != '') {
      ...
      $.ajax({
        ...
        success: function(data) {
          const term = $(searchField).val().toLowerCase().replace(/\s/g, '');
          // Test Valid Postcode
          let valid = false;
          let single = '';
          ...
          if (!valid) {
            ...
            if (single) {
              $('#address-select').empty().remove();
              let radios = '<div id=\'address-select\'><span>Please select your address:</span><ul>';
              $.each(data.Items, function(index, value) {
  ...
  function populateAddress(address) {
    ...
    // JQ DEP
    let inputElement = document.getElementById('add1');
