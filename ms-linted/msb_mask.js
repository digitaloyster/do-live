const msb_mask = function() {
  const masks = {'money decimal': {mask: '000,000,000', regex: RegExp(/^[£0-9,]+\.?\d{0,2}$/), frontChar: '£', reverse: true, escChar: {num: 1, key: '.'}},
  ...
  return {
    ...
    checkMask: function( inputVal ) {
      ...
      if ( a.frontChar && inputVal.length === 1 ) {
        this.oldSelectionStart ++;
  	    ...
  	    const buf = [];
      if ( a.reverse ) {
        const arrMethod = 'unshift';
        let caret = a.mask.length -1;
        let arrPos = this.unmaskArr.length - 1;
        while ( arrPos >= 0 ) {
          const maskVal = a.mask.charAt( caret );
          if ( caret >= 0 ) {
            const retVar = this.calcMask( caret, arrPos, maskVal, a );
            if ( retVar['retchar'] ) {
    ...
    calcMask: function( caret, arrPos, maskVal, a) {
      ...
      if ( a.escChar && this.unmaskArr[arrPos] == a.escChar.key && this.escCharNum < a.escChar.num) {
        ...
        if ( $.inArray( a.mask.charAt( caret ), this.maskCharArr ) === -1 ) {
          this.maskCharArr.push( a.mask.charAt( caret ) );
        }
