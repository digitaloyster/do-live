...
window.onload = function() {
  $('body').after('<style id="postcode-switch">'+
  '#container_postcode .error-message {display:none !important;}</style>');
  $('#address_search').change(function() {
    if ($('#address_search').val() != '' && $('#postcode').val() == '') {
      $('#container_address_search .error-message').remove();
      $('#postcode-switch')
          .text('#container_postcode .error-message {display:block !important;}');
    } else {
      $('#postcode-switch')
          .text('#container_postcode .error-message {display:none !important;}');
    }
  });
    ...
    showResults: function( pa, results ) {
      if (!results.Count) {
        ...
        $('#address_search').addClass('error')
            .after('<label for=\'address_search\' generated=\'true\' '+
        'class=\'error-message\' style=\'\'>'+
        'Please enter a valid address or postcode</label>');
        return true;
      } else {
        $('.error-message').remove();
      }
